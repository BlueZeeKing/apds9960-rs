use crate::{register::GStatus, Apds9960, BitFlags, Register};
    ...
    I2C: i2c::I2c<Error = E>,
{
    ...
    pub fn read_gesture_data_level(&mut self) -> Result<u8, E> {
        self.read_register(Register::GFLVL)
    }

    /// Read whether there is valid gesture data available.
    #[allow(clippy::wrong_self_convention)]
    pub fn is_gesture_data_valid(&mut self) -> Result<bool, E> {
    ...
    #[allow(clippy::wrong_self_convention)]
    pub fn has_gesture_data_overflown(&mut self) -> Result<bool, E> {
        let status = self.read_register(Register::GSTATUS)?;
    ...
    pub fn read_gesture_data(&mut self, data: &mut [u8]) -> nb::Result<(), E> {
